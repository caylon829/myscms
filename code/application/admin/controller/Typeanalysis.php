 ...
 * @icon fa fa-circle-o
 */
class Typeanalysis extends Backend
{
    
    /**
     * Typeanalysis模型对象
     * @var \app\admin\model\Typeanalysis
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\Typeanalysis;
        ...
        if(isset($_GET['filter'])&&$_GET['filter']!='{}'){
            ...
            if(isset($params['year'])&&isset($params['month'])){
                ...
                foreach ($searchtime as $key => $value){
                    ...
                    $result = Db::table('fa_bill')->field('sum(price) as sum,source_type')->where('period',$value)->group('source_type')->order('source_type','asc')->select();
                    $arrp=array();
                    foreach ($result as $kk =>$vv){
                        $arrp[]=$kk;
                        $list[$key]['sourcetype_'.$vv['source_type']]=$vv['sum'];
                    }

                    $diff=array_diff([0,1,2,3,4,5,6,7,8],$arrp);
                    foreach ($diff as $ii=>$jj){
                        $list[$key]['sourcetype_'.$jj]=0;
                    }
                }
                $total=count($list);
...
//            exit;
        ...
        if ($this->request->isAjax())
        {
            ...
            foreach ($list as $row) {
                $row->visible(['id','year','month','source_type_0','source_type_1','source_type_2','source_type_3','source_type_4','source_type_5','source_type_6','source_type_7','source_type_8']);
    ...
    public function supplier()
    {
        ...
        $result = Db::table('fa_bill')->field('sum(price) as sum,source_type,supplier_id')->where($where)->group('supplier_id')->order('source_type','asc')->select();
...
//            $endtime = $this->request->post('endtime');
        ...
        $str=['id'=>'序号','year'=>'年份','month'=>'月份','sourcetype_0' => '机柜',
            'sourcetype_1' => '带宽',
            'sourcetype_2' => 'IP地址',
            'sourcetype_3' => '传输',
            'sourcetype_4' => '电费',
...
//        exit;
        ...
        $excel->createSheet();
        $result = Db::table('fa_bill')->field('sum(price) as sum,source_type,supplier_id,period')->group('source_type,period,supplier_id')->order('source_type asc,period desc,supplier_id asc')->select();
        $supplier=Db::table('fa_supplier')->field('id,name')->select();
        $arrsupplier=array();
        foreach ($supplier as $value){
            $arrsupplier[$value['id']]=$value['name'];
        ...
        foreach ($result as $jkey=>$jvalue){
            $sub_result[$jvalue['period'].'-'.$str['sourcetype_'.$jvalue['source_type']]][$jkey]['name']=$arrsupplier[$jvalue['supplier_id']];
            $sub_result[$jvalue['period'].'-'.$str['sourcetype_'.$jvalue['source_type']]][$jkey]['sum']=$jvalue['sum'];
        }
//        var_dump($sub_result);
//        exit;
        $tempnum=1;
